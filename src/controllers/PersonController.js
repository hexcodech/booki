class PersonController{
	...
	constructor({
		...
		bindAll(this, [
			'getPerson',  'getPersonById', 'postPerson',
			'putPerson',  'deletePerson',  'lookupPerson'
		]);

	}

	getPerson(request, response, next){
    this.Person.findAll().then((people) => {
			if(people){
  ...
  getPersonById(request, response, next){
    this.Person.findById(request.params.personId).then((person) => {
  ...
  postPerson(request, response, next){
		let person = this.Person.build(this.pick(request.body.person, [
			'nameTitle', 'nameFirst', 'nameMiddle', 'nameLast'
		]));

		if(request.hasPermission('admin.person.hiddenData.write')){
			person.set(this.omitBy(this.pick(request.body.person, [
				'id', 'verified'
			]), this.isNil));
		}
  ...
  putPerson(request, response, next){
		this.Person.findById(request.params.personId).then((person) => {
			if(person){
				person.set(this.pick(request.body.person, [
					'nameTitle', 'nameFirst', 'nameMiddle', 'nameLast'
				]));
