class ConditionController{
	...
	constructor({
		...
		bindAll(this, [
			'getCondition',  'getConditionById',
      'postCondition', 'putCondition', 'deleteCondition'
		]);

	}

  getCondition(request, response, next){
    this.Condition.findAll().then((conditions) => {
			if(conditions){

				if(request.hasPermission('admin.condition.hiddenData.read')){
					response.json(conditions.map((condition) => {
  ...
  postCondition(request, response, next){
		let condition = this.Condition.build(this.pick(request.body.condition, [
			'key', 'priceFactor'
		]));

		if(request.hasPermission('admin.condition.hiddenData.write')){
			condition.set(this.omitBy(this.pick(request.body.condition, [
				'id'
			]), this.isNil));
		}
  ...
  putCondition(request, response, next){
		this.Condition.findById(request.params.conditionId).then((condition) => {
			if(condition){
				...
				condition.set(this.pick(request.body.condition, [
					'key', 'priceFactor'
				]));
